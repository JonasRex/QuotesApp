...
class ProfileResourceTest {
    ...
    void addRelation() {
        given()
                .header("Content-type", ContentType.JSON)
                .pathParam("id", p1.getId()).pathParam("quoteid", q1.getId())
                .put("/profile/addquote/{id}/{quoteid}")
                .then()
                .assertThat()
                .statusCode(HttpStatus.OK_200.getStatusCode())
                .body("id", equalTo(p1.getId()))
                .body("quoteDTOS", hasItems(hasEntry("id",q1.getId())));
    ...
    void removeRelation() {
        given()
                .header("Content-type", ContentType.JSON)
                .pathParam("id", p2.getId()).pathParam("item_id", q1.getId())
                .delete("/profile/removequote/{id}/{item_id}")
                .then()
                .assertThat()
                .statusCode(HttpStatus.OK_200.getStatusCode())
                .body("id", equalTo(p2.getId()))
                .body("quoteDTOS", empty());
    }
