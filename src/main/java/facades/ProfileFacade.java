...
public class ProfileFacade implements IFacade<Profile> {
    ...
    public Profile update(Profile profile) throws EntityNotFoundException {
        ...
        p.setQuotesList(profile.getQuotesList());
    ...
    public Profile addRelation(int id1, int id2) throws EntityNotFoundException {
        ...
        try {
            ...
            Quote quote = em.find(Quote.class, id2);
            if (quote == null) {
                throw new EntityNotFoundException("Quote with ID: " + id2 + " not found");
            }
    ...
    public long getCount() {
        EntityManager em = getEntityManager();
        try {
            return (long) em.createQuery("SELECT COUNT(p) FROM Profile p").getSingleResult();
        } finally {
            em.close();
        }
    }
