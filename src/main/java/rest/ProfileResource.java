...
public class ProfileResource {
    ...
    @PUT
    @Path("/addquote/{profileid}/{quote_id}")
    @Produces({MediaType.APPLICATION_JSON})
    public Response addRelation(@PathParam("profileid") int id1, @PathParam("quote_id") int id2) throws EntityNotFoundException {
        ProfileDTO updated = FACADE.addRelation(id1, id2);
        return Response.ok().entity(GSON.toJson(updated)).build();
    }

    @DELETE
    @Path("/removequote/{profileid}/{quote_id}")
    @Produces({MediaType.APPLICATION_JSON})
    public Response removeRelation(@PathParam("profileid") int id1, @PathParam("quote_id") int id2) throws EntityNotFoundException {
